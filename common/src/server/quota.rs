...
impl Drop for QuotaGuard {
    fn drop(&mut self) {
        ...
        if !self.committed {
            debug!(
                "QuotaGuard: Returning {} bytes for {} (not committed)",
                self.bytes, self.home_dir
            );
            // 注意：Drop 可能在非异步上下文中调用，所以使用 blocking_lock
...
impl QuotaCache {
    ...
    pub async fn get_usage(&self, home_dir: &str) -> u64 {
        ...
        drop(cache);
        // 注意：calculate_usage 内部也会获取锁，所以需要释放后再调用
        self.calculate_usage_async(home_dir).await
    }
    ...
    ) -> Result<QuotaGuard, &'static str> {
        // 配额为 0 表示无限制
        if quota_mb == 0 {
            return Ok(QuotaGuard::new(
                additional_bytes,
                home_dir.to_string(),
                self.cache.clone(),
            ));
    ...
    pub async fn check_quota(&self, home_dir: &str, quota_mb: u64, additional_bytes: u64) -> bool {
        ...
        let quota_bytes = quota_mb * 1024 * 1024;
        let current_usage = self.get_usage(home_dir).await;
