...
impl FtpSession {
    pub async fn cmd_pasv(&mut self) -> Result<()> {
        ...
        if self.remote_ip.contains(':') {
            self.stream
                .write_all(b"425 IPv6 addresses not supported in PASV mode, use EPSV instead\r\n")
                .await?;
        ...
        let passive_port =
            find_available_passive_port(&self.passive_listeners, port_min, port_max).await?;
...

        {
            ...
            listeners.insert(
                passive_port,
                Arc::new(tokio::sync::Mutex::new(Some(passive_listener))),
            );
        }
    ...
    pub async fn cmd_epsv(&mut self) -> Result<()> {
        ...
        let (port_min, port_max, bind_ip) = {
            let cfg = self.config.lock().unwrap();
            (
                cfg.ftp.passive_ports.0,
                cfg.ftp.passive_ports.1,
                cfg.ftp.bind_ip.clone(),
            )
        };
        ...
        if let Some(data) = arg {
            ...
            if parts.len() == 6 {
                ...
                if client_specified_ip != self.remote_ip {
                    ...
                    self.stream
                        .write_all(b"500 Illegal PORT command - IP must match client IP\r\n")
                        .await?;
                    return Ok(());
                }
            ...
            self.stream
                .write_all(b"501 Syntax error: PORT requires parameters\r\n")
                .await?;
        }
        Ok(())
    }
        ...
        if let Some(data) = arg {
            ...
            if parts.len() >= 4 {
                ...
                if net_addr != self.remote_ip {
                    ...
                    self.stream
                        .write_all(b"500 Illegal EPRT command - IP must match client IP\r\n")
                        .await?;
                    return Ok(());
                }
                ...
                match net_proto {
                    "1" => {
                        if let Ok(port) = tcp_port.parse::<u16>() {
                            ...
                            self.stream
                                .write_all(b"501 Invalid port number\r\n")
                                .await?;
                        }
                    }
                    "2" => {
                        if let Ok(port) = tcp_port.parse::<u16>() {
                            ...
                            self.passive_mode = false;
                            self.stream
                                .write_all(b"200 EPRT command successful (IPv6)\r\n")
                                .await?;
                        } else {
                            self.stream
                                .write_all(b"501 Invalid port number\r\n")
                    ...
                    _ => {
                        self.stream
                            .write_all(b"522 Protocol not supported, use (1,2)\r\n")
                            .await?;
                    }
                }
            } else {
                self.stream
                    .write_all(b"501 Syntax error in EPRT parameters\r\n")
                    .await?;
            }
        } else {
            self.stream
                .write_all(b"501 Syntax error: EPRT requires parameters\r\n")
                .await?;
        }
        Ok(())
    }
