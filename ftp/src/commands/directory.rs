...
use super::super::utils::{
    build_mlst_facts, escape_mlst_filename, real_to_virtual_path, safe_resolve_path,
};
...
impl FtpSession {
    pub async fn cmd_cwd(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(dir) = arg {
            ...
            if new_path.exists() && new_path.is_dir() && new_path.starts_with(home_path) {
                ...
                self.stream
                    .write_all(
                        format!("250 \"{}\" is current directory\r\n", virtual_path).as_bytes(),
                    )
                    .await?;
            } else {
    ...
    pub async fn cmd_cdup(&mut self) -> Result<()> {
        ...
        if new_path.starts_with(home_path) && new_path.exists() {
            ...
            self.stream
                .write_all(b"550 Cannot change to parent directory: Permission denied\r\n")
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn cmd_mlst(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if target_path.exists() && target_path.starts_with(home_path) {
            if let Ok(metadata) = target_path.metadata() {
                ...
                let virtual_path =
                    real_to_virtual_path(&target_path.to_string_lossy(), &self.home_dir);
                let escaped_name = escape_mlst_filename(&virtual_path);
                self.stream
                    .write_all(
                        format!(
                            "250-Listing {}\r\n {}{}\r\n250 End\r\n",
                            virtual_path, facts, escaped_name
                        )
                        .as_bytes(),
                    )
                    .await?;
                ...
                self.stream
                    .write_all(b"550 Failed to get file info\r\n")
                    .await?;
            }
        } else {
    ...
    pub async fn cmd_mkd(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(dirname) = arg {
            ...
            if std::fs::create_dir_all(&dir_path).is_ok() {
                let virtual_path =
                    real_to_virtual_path(&dir_path.to_string_lossy(), &self.home_dir);
                self.stream
                    .write_all(format!("257 \"{}\" created\r\n", virtual_path).as_bytes())
                    .await?;
                ...
                self.stream
                    .write_all(b"550 Create directory operation failed\r\n")
                    .await?;
            }
        }
        Ok(())
    ...
    pub async fn cmd_rmd(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(dirname) = arg {
            ...
            if dir_path == cwd_path {
                self.stream
                    .write_all(b"550 Cannot remove current working directory\r\n")
                    .await?;
                return Ok(());
            }
            ...
            if cwd_path.starts_with(&dir_path) && cwd_path != dir_path {
                self.stream
                    .write_all(b"550 Cannot remove parent of current working directory\r\n")
                    .await?;
                return Ok(());
            }
            ...
            if std::fs::remove_dir_all(&dir_path).is_ok() {
                ...
                self.stream
                    .write_all(b"550 Remove directory operation failed\r\n")
                    .await?;
            }
        }
        Ok(())
