...
impl FtpSession {
    pub async fn cmd_help(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(cmd) = arg {
            ...
            self.stream
                ...
                .await?;
            self.stream
                .write_all(b"214-USER PASS CWD CDUP PWD LIST NLST RETR STOR\r\n")
                .await?;
            self.stream
                .write_all(b"214-DELE MKD RMD RNFR RNTO PASV EPSV PORT EPRT\r\n")
                .await?;
            self.stream
                .write_all(b"214-TYPE MODE STRU REST SIZE MDTM ABOR QUIT\r\n")
                .await?;
            self.stream
                .write_all(b"214-MLSD MLST SYST FEAT STAT HELP NOOP\r\n")
                .await?;
            self.stream
                .write_all(b"214 Direct comments to admin\r\n")
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn cmd_mode(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(mode) = arg {
            match mode.to_uppercase().as_str() {
                ...
                "B" => {
                    self.stream
                        .write_all(b"504 Block mode not supported\r\n")
                        .await?
                }
                "C" => {
                    self.stream
                        .write_all(b"504 Compressed mode not supported\r\n")
                        .await?
                }
            ...
            self.stream
                .write_all(b"501 Syntax error: MODE requires parameter\r\n")
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn cmd_stru(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(structure) = arg {
            match structure.to_uppercase().as_str() {
                "F" => {
                    self.stream
                        .write_all(b"200 Structure set to File\r\n")
                        .await?
                }
                "R" => {
                    self.stream
                        .write_all(b"504 Record structure not supported\r\n")
                        .await?
                }
                "P" => {
                    self.stream
                        .write_all(b"504 Page structure not supported\r\n")
                        .await?
                }
            ...
            self.stream
                .write_all(b"501 Syntax error: STRU requires parameter\r\n")
                .await?;
        }
        Ok(())
    }
        ...
        if let Some(type_code) = arg {
            match type_code.to_uppercase().as_str() {
                "I" | "L 8" => {
                    ...
                    self.stream
                        .write_all(b"200 Type set to I (Binary)\r\n")
                        .await?;
                }
                "A" | "A N" => {
                    self.binary_transfer = false;
                    self.stream
                        .write_all(b"200 Type set to A (ASCII)\r\n")
                        .await?;
                }
                "E" => {
                    self.binary_transfer = true;
                    self.stream
                        .write_all(b"200 Type set to E (EBCDIC)\r\n")
                        .await?;
                }
    ...
    pub async fn cmd_rest(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(offset_str) = arg {
            if let Ok(offset) = offset_str.parse::<u64>() {
                ...
                self.stream
                    .write_all(b"501 Syntax error in REST parameter\r\n")
                    .await?;
            }
        } else {
            self.rest_offset = 0;
    ...
    pub async fn cmd_stat(&mut self) -> Result<()> {
        if let Some(ref username) = self.current_user {
            ...
            self.stream
                .write_all(format!("211-Connected to: {}\r\n", self.remote_ip).as_bytes())
                .await?;
            self.stream
                .write_all(format!("211-Logged in as: {}\r\n", username).as_bytes())
                .await?;
            self.stream
                .write_all(format!("211-Current directory: {}\r\n", self.cwd).as_bytes())
                .await?;
            self.stream
                .write_all(
                    format!(
            ...
            self.stream
                .write_all(b"211 FTP server status - Not logged in\r\n")
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn cmd_opts(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(opts_arg) = arg {
            ...
            if parts.is_empty() {
                self.stream
                    .write_all(b"501 Syntax error: OPTS requires parameters\r\n")
                    .await?;
                return Ok(());
            }
            ...
            match parts[0].to_uppercase().as_str() {
                ...
                "MLST" => {
                    self.stream
                        .write_all(b"200 MLST OPTS Type*;Size*;Modify*;\r\n")
                        .await?;
                }
                _ => {
                    self.stream
                        .write_all(b"501 Unknown OPTS parameter\r\n")
                        .await?;
                }
            }
        } else {
    ...
    pub async fn cmd_site(&mut self, arg: Option<&str>) -> Result<()> {
        if let Some(site_arg) = arg {
            ...
            if parts.is_empty() {
                self.stream
                    .write_all(b"501 Syntax error: SITE requires parameters\r\n")
                    .await?;
                return Ok(());
            }
            ...
            match parts[0].to_uppercase().as_str() {
                "CHMOD" => {
                    if parts.len() >= 3 {
                        self.stream
                            .write_all(b"200 SITE CHMOD command accepted\r\n")
                            .await?;
                    } else {
                        self.stream
                            .write_all(b"501 Syntax error: SITE CHMOD requires mode and path\r\n")
                            .await?;
                    }
                }
                "UMASK" => {
                    if parts.len() >= 2 {
                        self.stream
                            .write_all(b"200 SITE UMASK command accepted\r\n")
                            .await?;
                    } else {
                        self.stream
                            .write_all(b"501 Syntax error: SITE UMASK requires mask\r\n")
                            .await?;
                    }
                }
                "HELP" => {
                    self.stream
                        .write_all(b"214-SITE commands: CHMOD, UMASK, HELP\r\n214 End\r\n")
                        .await?;
                }
                _ => {
                    self.stream
                        .write_all(b"500 Unknown SITE command\r\n")
                        .await?;
                }
            }
        } else {
