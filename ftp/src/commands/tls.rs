...
impl FtpSession {
    pub async fn cmd_auth(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if !self.is_tls_available() {
            self.stream
                .write_all(b"500 TLS not configured on server\r\n")
                .await?;
            return Ok(());
        }
        ...
        match protocol.as_str() {
            "TLS" | "TLS-C" | "SSL" | "TLS-P" => {
                self.stream
                    .write_all(b"234 OK, starting TLS negotiation\r\n")
                    .await?;

                let tls_acceptor =
                    TlsAcceptor::from(self.tls_server_config.as_ref().unwrap().clone());
            ...
            _ => {
                self.stream
                    .write_all(b"504 Unknown authentication method\r\n")
                    .await?;
            }
        }
    ...
    pub async fn cmd_pbsz(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(size_str) = arg {
            match size_str.parse::<u64>() {
                ...
                Ok(_) => {
                    self.stream
                        .write_all(b"200 PBSZ=0 (only 0 supported for TLS)\r\n")
                        .await?;
                    self.pbsz_set = true;
                }
                Err(_) => {
                    self.stream
                        .write_all(b"501 Invalid PBSZ parameter\r\n")
                        .await?;
                }
            }
        } else {
            self.stream
                .write_all(b"501 PBSZ requires parameter\r\n")
                .await?;
        }
        ...
        if let Some(level) = arg {
            match level.to_uppercase().as_str() {
                "P" => {
                    ...
                    self.stream
                        .write_all(b"200 PROT Private - Data channel secured\r\n")
                        .await?;
                    info!(
                        username = self.current_user.as_deref().unwrap_or("anonymous"),
                        client_ip = %self.remote_ip,
                ...
                "C" => {
                    ...
                    self.stream
                        .write_all(b"200 PROT Clear - Data channel unsecured\r\n")
                        .await?;
                }
                "S" | "E" => {
                    self.stream
                        .write_all(b"536 PROT level not supported (use P or C)\r\n")
                        .await?;
                }
                _ => {
            ...
            self.stream
                .write_all(b"501 PROT requires parameter\r\n")
                .await?;
        }
    ...
    pub async fn cmd_ccc(&mut self) -> Result<()> {
        ...
        self.stream
            .write_all(b"200 CCC OK - reverting to clear text\r\n")
            .await?;
