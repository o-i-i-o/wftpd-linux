...
use super::super::utils::{
    build_mlst_facts, escape_mlst_filename, get_file_mtime, safe_resolve_path,
};
...
impl FtpSession {
    pub async fn cmd_nlst(&mut self, _arg: Option<&str>) -> Result<()> {
        ...
        match data_result {
            ...
            Err(e) => {
                ...
                self.cleanup_data_connection().await;
                self.stream
                    .write_all(b"425 Cannot open data connection\r\n")
                    .await?;
            }
        }
    ...
    pub async fn cmd_retr(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(filename) = arg {
            ...
            if !file_path.exists()
                || !file_path.is_file()
                || !file_path.starts_with(Path::new(&self.home_dir))
            {
                warn!(
                                ...
                                Ok(n) => {
                                    ...
                                    if let Some(limit_kbps) = effective_speed_limit {
                                        ...
                                        if elapsed >= Duration::from_millis(100) {
                                            let max_bytes =
                                                (limit_kbps * 1024) as f64 * elapsed.as_secs_f64();
                                            if bytes_since_throttle > max_bytes as u64 {
                                                let excess =
                                                    bytes_since_throttle - max_bytes as u64;
                                                let wait_secs =
                                                    excess as f64 / (limit_kbps * 1024) as f64;
        ...
        if let Some(filename) = arg {
            ...
            if quota_mb > 0 {
                ...
                if current_usage >= quota_bytes {
                    ...
                    self.stream
                        .write_all(b"552 Quota exceeded, upload denied\r\n")
                        .await?;
                    return Ok(());
                }
            }
                                ...
                                match data_stream.read(&mut buf).await {
                                    ...
                                    Ok(n) => {
                                        ...
                                        if let Some(limit_kbps) = effective_speed_limit {
                                            ...
                                            let elapsed = last_throttle_time.elapsed();
                                            if elapsed >= Duration::from_millis(100) {
                                                let max_bytes = (limit_kbps * 1024) as f64
                                                    * elapsed.as_secs_f64();
                                                if bytes_since_throttle > max_bytes as u64 {
                                                    let excess =
                                                        bytes_since_throttle - max_bytes as u64;
            ...
            if transfer_success {
                ...
                let uploaded_size = std::fs::metadata(&file_path)
                    .map(|m| m.len())
    ...
    pub async fn cmd_appe(&mut self, arg: Option<&str>) -> Result<()> {
        ...
        if let Some(filename) = arg {
            ...
            self.stream
                .write_all(b"150 Opening BINARY mode data connection for append\r\n")
                .await?;
    ...
    pub async fn cmd_stou(&mut self, _arg: Option<&str>) -> Result<()> {
            ...
            Ok(mut data_stream) => {
                ...
                match File::create(&file_path).await {
                    Ok(mut file) => {
                        ...
                        if transfer_success && let Err(e) = file.sync_all().await {
        ...
        if transfer_success {
            self.stream
                .write_all(format!("226 Transfer complete, file: {}\r\n", unique_name).as_bytes())
                .await?;
