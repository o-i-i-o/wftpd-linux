...
impl FtpStream {
    pub async fn write_all(&mut self, buf: &[u8]) -> std::io::Result<()> {
        match self {
            ...
            FtpStream::Taken => Err(std::io::Error::new(
...
impl FtpSession {
    pub fn new(stream: TcpStream, session_config: FtpSessionConfig) -> Result<Self> {
        ...
        let local_ip = stream.local_addr().ok().map(|addr| addr.ip().to_string());
    ...
    async fn handle_command(&mut self, cmd: &str, arg: Option<&str>) -> Result<bool> {
        match cmd {
            ...
            "ALLO" => {
                self.stream
                    .write_all(b"200 ALLO command successful\r\n")
                    .await?
            }
