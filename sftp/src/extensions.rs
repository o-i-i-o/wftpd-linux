...
impl SftpState {
    pub async fn handle_extended(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        if data.len() < 9 {
            return Ok(build_status_packet(
                0,
                SSH_FX_FAILURE,
                "Invalid extended packet: too short",
                "",
            ));
        }
        ...
        if ext_name_len_pos + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid extended packet: extension name length overflow",
                "",
            ));
        }
        ...
        if remaining_data < 4 && ext_name != "limits@openssh.com" {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid extended packet: missing data",
                "",
            ));
        }
        ...
        match ext_name.as_str() {
            ...
            "md5sum@openssh.com" | "md5-hash@openssh.com" => {
                self.handle_md5sum(id, data, ext_total_len).await
            }
            "sha256sum@openssh.com" | "sha256-hash@openssh.com" => {
                self.handle_sha256sum(id, data, ext_total_len).await
            }
    ...
    async fn handle_statvfs(&self, id: u32, data: &[u8], ext_offset: usize) -> Result<Vec<u8>> {
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid statvfs packet: missing path length",
                "",
            ));
        }
            ...
            && end > data.len()
        {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid statvfs packet: path data truncated",
                "",
            ));
        }
    ...
    async fn handle_fstatvfs(&self, id: u32, data: &[u8], ext_offset: usize) -> Result<Vec<u8>> {
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid fstatvfs packet: missing handle length",
                "",
            ));
        }
            ...
            && end > data.len()
        {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid fstatvfs packet: handle data truncated",
                "",
            ));
        }
        ...
        match self.handles.get(&handle_str) {
            Some(handle) if !handle.is_dir => {
                let file = handle
                    .file
                    .as_ref()
                    .ok_or_else(|| anyhow::anyhow!("File handle is None"))?;
                match fstatvfs(file) {
                    Ok(stats) => Ok(build_statvfs_reply(id, &stats)),
                    Err(e) => {
                        let (status, msg) = match e {
            ...
            Some(_) => Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Handle does not reference a file",
                "",
            )),
            None => Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid handle",
                "",
    ...
    async fn handle_fsync(&self, id: u32, data: &[u8], ext_offset: usize) -> Result<Vec<u8>> {
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid fsync packet: missing handle length",
                "",
            ));
        }
            ...
            && end > data.len()
        {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid fsync packet: handle data truncated",
                "",
            ));
        }
    ...
    async fn handle_md5sum(&self, id: u32, data: &[u8], ext_offset: usize) -> Result<Vec<u8>> {
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid md5sum packet: missing path length",
                "",
            ));
        }
            ...
            && end > data.len()
        {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid md5sum packet: path data truncated",
                "",
            ));
        }
    ...
    async fn handle_sha256sum(&self, id: u32, data: &[u8], ext_offset: usize) -> Result<Vec<u8>> {
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid sha256sum packet: missing path length",
                "",
            ));
        }
            ...
            && end > data.len()
        {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid sha256sum packet: path data truncated",
                "",
            ));
        }
    ...
    async fn handle_copy_file(
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid copy-file packet: missing source path length",
                "",
            ));
        }
        ...
        let src_end = parse_offset
            .checked_add(4 + src_len)
            .ok_or_else(|| anyhow::anyhow!("Source path end overflow"))?;
        if src_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid copy-file packet: source path data truncated",
                "",
            ));
        }
        ...
        if dst_pos + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid copy-file packet: missing destination path length",
                "",
            ));
        }
        ...
        let (dst_path, dst_len) = match parse_string_checked(data, dst_pos) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("Invalid destination path: {}", e),
                    "",
                ));
            }
        };
        ...
        let dst_end = dst_pos
            .checked_add(4 + dst_len)
            .ok_or_else(|| anyhow::anyhow!("Destination path end overflow"))?;
        if dst_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid copy-file packet: destination path data truncated",
                "",
            ));
        }
        ...
        let src_full = match self.resolve_path(&src_path) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("Source path resolution failed: {}", e),
                    "",
                ));
            }
        };
        let dst_full = match self.resolve_path(&dst_path) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("Destination path resolution failed: {}", e),
                    "",
                ));
            }
        };
        ...
        if !self
            .check_quota_for_additional_bytes(additional_bytes)
            .await
        {
            return Ok(build_status_packet(
                id,
    ...
    async fn handle_hardlink(
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid hardlink packet: missing source path length",
                "",
            ));
        }
        ...
        if src_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid hardlink packet: source path data truncated",
                "",
            ));
        }
        ...
        if dst_pos + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid hardlink packet: missing destination path length",
                "",
            ));
        }
        ...
        if dst_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid hardlink packet: destination path data truncated",
                "",
            ));
        }
    ...
    async fn handle_posix_rename(
        ...
        if parse_offset + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid posix-rename packet: missing old path length",
                "",
            ));
        }
        ...
        let (old_path, old_len) = match parse_string_checked(data, parse_offset) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("Invalid old path: {}", e),
                    "",
                ));
            }
        };

        let old_end = parse_offset
            .checked_add(4 + old_len)
            .ok_or_else(|| anyhow::anyhow!("Old path end overflow"))?;
        if old_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid posix-rename packet: old path data truncated",
                "",
            ));
        }
        ...
        if new_path_pos + 4 > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid posix-rename packet: missing new path length",
                "",
            ));
        }
        ...
        let new_end = new_path_pos
            .checked_add(4 + new_len)
            .ok_or_else(|| anyhow::anyhow!("New path end overflow"))?;
        if new_end > data.len() {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid posix-rename packet: new path data truncated",
                "",
            ));
        }
        ...
        let old_full = match self.resolve_path(&old_path) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("Old path resolution failed: {}", e),
                    "",
                ));
            }
        };
        ...
        let new_full = match self.resolve_path(&new_path) {
            ...
            Err(e) => {
                return Ok(build_status_packet(
                    id,
                    SSH_FX_FAILURE,
                    &format!("New path resolution failed: {}", e),
                    "",
                ));
            }
        };
