...
impl SftpHandler {
    pub fn new(
        ...
        info!(
            "[SFTP HANDLER] Creating new SFTP handler for client {}",
            client_ip
        );
        // [DEBUG] 添加详细的连接信息
        tracing::debug!(
            "[SFTP DEBUG] Handler created - users_path={:?}, keys_dir={:?}",
            users_path,
            keys_dir
        );
    ...
    async fn validate_and_set_home_dir(
        ...
        match tokio::fs::metadata(&home).await {
            ...
            Err(e) => {
                return Err(format!(
                    "home directory '{}' does not exist or cannot be accessed: {}",
                    home_dir, e
                ));
            }
        }
        ...
        match tokio::fs::canonicalize(&home).await {
            ...
            Err(e) => Err(format!(
                "cannot canonicalize home directory '{}': {}",
                home.display(),
                e
            )),
        }
...
impl russh::server::Handler for SftpHandler {
    ...
    async fn auth_password(
        ...
        // [DEBUG] 添加详细的认证日志
        tracing::debug!(
            "[SFTP AUTH DEBUG] auth_password called - user={}, password_len={}",
            user,
            password.len()
        );
            ...
            match self.user_manager.try_lock() {
                Ok(mut users) => {
                    ...
                    for (username, user) in users.get_users().iter() {
                        tracing::debug!(
                            "[SFTP AUTH DEBUG] Available user: {}, enabled={}, home={}",
                            username,
                            user.enabled,
                            user.home_dir
                        );
    ...
    async fn auth_publickey(
        ...
        if !enabled {
            ...
            self.log_client_action(
                "SFTP",
                &format!(
                    "Public key auth failed for user {}: user not found or disabled",
                    user
                ),
                Some(user),
                "AUTH_FAIL",
        ...
        match tokio::fs::read_to_string(&user_pubkey_path).await {
            Ok(stored_key) => match keys::parse_public_key_base64(stored_key.trim()) {
                Ok(stored_pubkey) => {
        ...
        self.log_client_action(
            "SFTP",
            &format!(
                "Public key auth failed for user {}: key mismatch or not found",
                user
            ),
            Some(user),
            "AUTH_FAIL",
    ...
    async fn auth_succeeded(&mut self, _session: &mut Session) -> Result<(), Self::Error> {
        ...
        // [DEBUG] 添加详细的认证成功日志
        tracing::debug!(
            "[SFTP DEBUG] auth_succeeded called - authenticated={}, username={:?}",
            self.authenticated,
            self.username
        );
        Ok(())
    ...
    async fn subsystem_request(
        ...
        // [DEBUG] 添加详细的子系统请求日志
        tracing::debug!(
            "[SFTP DEBUG] subsystem_request - name={}, authenticated={}, home_dir={:?}",
            name,
            self.authenticated,
            self.home_dir
        );
        ...
        self.log_info(&format!(
            "SFTP subsystem initialized for user: {:?}, home_dir: {}",
            username, home_dir
        ));
