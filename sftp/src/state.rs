...
use wftpd_common::server::utils::{
    safe_resolve_path, validate_path_for_creation, validate_path_within_chroot,
};
...
impl Drop for SftpState {
    fn drop(&mut self) {
        let locked_handles: Vec<PathBuf> = self
            .handles
            .drain()
            .filter_map(|(_, handle)| {
                if handle.locked {
...
fn format_longname(name: &str, is_dir: bool, size: u64, mtime: i64, mode: u32) -> String {
    ...
    let datetime =
        chrono::DateTime::from_timestamp(mtime, 0).unwrap_or(chrono::DateTime::UNIX_EPOCH);
...
impl SftpState {
    pub fn new(
        ...
        let cached_permissions = username.as_ref().and_then(|u| {
            user_manager
                .lock()
                .ok()
                .and_then(|mgr| mgr.get_user(u).map(|user| user.permissions))
        });
    ...
    pub async fn process_sftp_data(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if self.buffer.len() > MAX_BUFFER_SIZE {
            warn!(
                ...
                "[SFTP] Buffer size exceeded limit, clearing"
            );
            self.buffer.clear();
        ...
        while self.buffer.len() >= 4 {
            ...
            if packet_len > MAX_PACKET_SIZE {
                warn!(
                    ...
                    "[SFTP] Packet size exceeds maximum"
                );
                self.buffer.clear();
                return Ok(build_status_packet(
                    0,
    ...
    async fn handle_close(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if let Some(handle) = self.handles.remove(&handle_str)
            && !handle.is_dir
            && handle.written_bytes > 0
        {
            let file_size = tokio::fs::metadata(&handle.path)
                .await
                .map(|m| m.len())
                .unwrap_or(handle.written_bytes);
        ...
        match handle {
            Some(h) if h.is_dir => {
                ...
                let result_entries: Vec<(String, bool, u64)> =
                    h.dir_entries[h.dir_index..h.dir_index + count].to_vec();
                h.dir_index += count;
    ...
    async fn handle_read(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if data.len() < offset_pos + 12 {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid packet: too short for read",
                "",
            ));
        }
    ...
    async fn handle_write(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if data.len() < offset_pos + 12 {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid packet: too short for write",
                "",
            ));
        }
        ...
        if data.len() < data_start + data_len {
            return Ok(build_status_packet(
                id,
                SSH_FX_FAILURE,
                "Invalid packet: write data truncated",
                "",
            ));
        }
        ...
        let current_len = tokio::fs::metadata(&target_path)
            .await
    ...
    async fn handle_setstat(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if flags & 0x00000004 != 0 && data.len() >= offset + 4 {
            ...
            debug!(
                "SETSTAT: requested permissions=0o{:o} for {:?}",
            ...
            #[cfg(unix)]
            {
                ...
                if permissions & 0o7000 != 0 {
                    warn!(
                        "SECURITY: User attempted to set special permission bits: 0o{:o}",
                        permissions
                    );
                    return Ok(build_status_packet(
                        id,
                ...
                */
                ...
                debug!(
                    "SETSTAT: setting mode=0o{:o} (filtered from 0o{:o})",
                    mode, permissions
                );

                match tokio::fs::set_permissions(&full_path, std::fs::Permissions::from_mode(mode))
                    .await
                {
                    Ok(_) => {
        ...
        match handle {
            Some(h) if !h.is_dir => {
                ...
                if flags & 0x00000004 != 0 && data.len() >= offset + 4 {
                    let permissions = parse_u32_checked(data, offset)?;
                    debug!(
                        "FSETSTAT: requested permissions=0o{:o} for {:?}",
                        permissions, h.path
                    );
                    #[cfg(unix)]
                    {
                        ...
                        debug!(
                            "FSETSTAT: setting mode=0o{:o} (filtered from 0o{:o})",
                            mode, permissions
                        );
                        ...
                        .await
                        {
                            Ok(_) => {
                                debug!(
                                    "FSETSTAT: permissions successfully changed to 0o{:o}",
                                    mode
                                );
                            }
                            Err(e) => {
    ...
    async fn handle_fstat(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let handle = self.handles.get(&handle_str);
        match handle {
            Some(h) if !h.is_dir => match tokio::fs::metadata(&h.path).await {
                Ok(metadata) => {
                    let mut payload = vec![105];
                    payload.extend_from_slice(&id.to_be_bytes());
                    payload.extend_from_slice(&build_attrs(metadata.is_dir(), metadata.len()));
    ...
    async fn handle_symlink(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let full_target = match validate_path_within_chroot(&target, &self.home_dir).await {
            ...
            Err(e) => {
                ...
                return Ok(build_status_packet(
                    id,
                    SSH_FX_PERMISSION_DENIED,
                    &format!("Invalid symlink target: {}", e),
                    "",
                ));
            }
        };
        ...
        debug!(
            "SYMLINK: Creating link {:?} -> {:?}",
            full_link, full_target
        );
        ...
        match std::os::unix::fs::symlink(&full_target, &full_link) {
            Ok(_) => {
                debug!(
                    "SYMLINK: Successfully created symlink {:?} -> {:?}",
                    full_link, full_target
                );
                ...
                match std::fs::symlink_metadata(&full_link) {
                    Ok(metadata) => {
                        ...
                        debug!(
                            "SYMLINK: Verification - is_symlink={}, mode={:o}",
                            is_symlink,
                            metadata.mode()
                        );
                            ...
                            Ok(link_target) => {
                                if link_target.is_absolute() {
                                    ...
                                    if !link_target.starts_with(&home_canon) {
                                        warn!(
                                            "SYMLINK: Symlink points outside chroot: {:?}",
                                            link_target
                                        );
                                        let _ = std::fs::remove_file(&full_link);
                                        return Ok(build_status_packet(
                                            id,
                                            SSH_FX_PERMISSION_DENIED,
                                            "Symlink target outside home directory",
                                            "",
                                        ));
                                    }
                                }
                ...
                if let Ok(mut fl) = self.file_logger.lock() {
                    fl.log(wftpd_common::FileLogInfo {
...
mod tests {
    ...
    use wftpd_common::{Permissions, UserManager};
    ...
    fn make_state(home: &Path, permissions: Permissions) -> SftpState {
        ...
        SftpState::new(
            ...
            Arc::new(StdMutex::new(FileLogger::new(
    ...
    async fn init_advertises_supported_extensions() {
        ...
        let response = state
            .handle_init(&[SSH_FXP_INIT, 0, 0, 0, 3])
            .await
            .unwrap();
        ...
        assert!(
            extensions
                .iter()
                .any(|(name, _)| name == "limits@openssh.com")
        );
        assert!(extensions.iter().any(|(name, _)| name == "copy-file"));
        assert!(
            extensions
                .iter()
                .any(|(name, _)| name == "fsync@openssh.com")
        );
        assert!(
            extensions
                .iter()
                .any(|(name, _)| name == "fstatvfs@openssh.com")
        );
    }
    ...
    async fn fsync_and_fstatvfs_succeed_for_file_handle() {
        ...
        let fsync_response = state
            .handle_extended(&build_extended_packet(
                9,
                "fsync@openssh.com",
                &build_string_field("h00000002"),
            ))
            .await
            .unwrap();
